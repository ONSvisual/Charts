...
export function addDirectionArrow(svgName,
  ...
  if (editable) {
    // const baseMarker = svgName.append("circle")
    //   .attr("class", `annotation-marker base-marker ${annotationId}`)
    //   .attr("cx", xValue)  // Use original coordinates for the marker
    //   .attr("cy", yValue)
    //   .attr("r", 3)
    //   .style("fill", "none")
    //   .style("stroke", "darkgreen")
...
function setDirectionArrow(direction = 'right', arrowAnchor = 'start') {
  ...
  const paths = {
    // up:"M5.5 1.5L5.5 12.5M5.5 1.5L9.5 5.5M5.5 1.5L1.5 5.5",
    // down:"M5.5 12.5L5.5 1.5M5.5 12.5L1.5 8.5M5.5 12.5L9.5 8.5",
 ...
 * @param {number} [config.text.adjustY] - Y adjustment for text for direction and elbow arrows
  ...
  switch (type) {
    ...
    case 'elbow-arrow':
      return addElbowArrow(
        ...
        label, txt.wrapWidth, txt.adjustY, txt.verticalAlign, sty.color,
        pos.text, editable
      );
