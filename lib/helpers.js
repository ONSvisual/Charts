...
export function createAdvancedDelaunayOverlay({
  svgContainer,
  data,
  chart_width,
  height,
  xScale,
  yScale,
  tooltipTemplate = null, // Custom tooltip template function
  highlightConfig = {},
  tooltipConfig = {},
  onPointHover = null, // Callback function for point hover
  onPointLeave = null  // Callback function for point leave
}) {
  ...
  const defaultTooltipConfig = {
    ...
    fontSize: '14px',
    offset: { x: 10, y: -10 },
    className: 'scatter-tooltip-advanced'
  };

  const highlightStyle = { ...defaultHighlightConfig, ...highlightConfig };
  const tooltipStyle = { ...defaultTooltipConfig, ...tooltipConfig };
  ...
  const tooltip = d3.select('body')
    .append('div')
    .attr('class', tooltipStyle.className)
    .style('position', 'absolute')
    .style('visibility', 'hidden')
    ...
    .style('padding', tooltipStyle.padding)
  ...
  const overlay = svgContainer
    .append('rect')
    .attr('class', 'delaunay-overlay-advanced')
    .attr('width', chart_width)
    .attr('height', height)
    .attr('fill', 'transparent')
    .style('cursor', 'crosshair');
  ...
  const defaultTooltipTemplate = (point) => `
    <div style="font-weight: bold; margin-bottom: 4px;">${point.name}</div>
    <div style="margin-bottom: 2px;"><strong>Group:</strong> ${point.group}</div>
    <div style="margin-bottom: 2px;"><strong>X:</strong> ${tooltipStyle.xValueFormat(point.xvalue)}</div>
    <div><strong>Y:</strong> ${tooltipStyle.yValueFormat(point.yvalue)}</div>
  `;

  const templateFunction = tooltipTemplate || defaultTooltipTemplate;
  ...
  overlay.on('mousemove', function (event) {
    ...
    if (currentHighlight !== nearestIndex) {
      // Remove previous highlight
      svgContainer.selectAll(`.${highlightStyle.className}`).remove();

      // Add new highlight
      svgContainer
        .append('circle')
        .attr('class', highlightStyle.className)
        .attr('cx', xScale(nearestPoint.xvalue))
        .attr('cy', yScale(nearestPoint.yvalue))
        .attr('r', highlightStyle.radius)
        .attr('fill', highlightStyle.fill)
        .attr('stroke', highlightStyle.stroke)
        .attr('stroke-width', highlightStyle.strokeWidth)
        .style('pointer-events', 'none');

      // Update tooltip
      tooltip.html(templateFunction(nearestPoint));

      // Call hover callback if provided
      if (onPointHover) {
        onPointHover(nearestPoint, nearestIndex, event);
      }
    ...
    const pointY = yScale(nearestPoint.yvalue);
    tooltip
      .style('left', (containerRect.left + pointX + tooltipStyle.offset.x) + 'px')
      .style('top', (containerRect.top + pointY + tooltipStyle.offset.y) + 'px')
      .style('visibility', 'visible');
  });
  ...
  overlay.on('mouseleave', function (event) {
    ...
    if (onPointLeave && currentHighlight !== null) {
      onPointLeave(data[currentHighlight], currentHighlight, event);
    }

    currentHighlight = null;
  });

  // Return object with cleanup and utility methods
  return {
    cleanup: function () {
      tooltip.remove();
      overlay.remove();
    ...
    updateData: function (newData) {
      data = newData;
      // Recreate Delaunay with new data
      const newDelaunay = d3.Delaunay.from(
        newData,
        d => xScale(d.xvalue),
        d => yScale(d.yvalue)
      );
