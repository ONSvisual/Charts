export function calculateChartWidth({ screenWidth, chartEvery, chartMargin }) {
  if (config.optional.dropYAxis) {
    // Chart width calculation allowing for 10px left margin between the charts
    const chartWidth = ((screenWidth - chartMargin.left - ((chartEvery - 1) * 10)) / chartEvery) - chartMargin.right;
    return chartWidth;
  } else {
...
export function addDataLabels({ svgContainer = svg, data, chart_width, labelPositionFactor = 7, xScaleFunction = x, yScaleFunction = y }) {
  ...
  svgContainer
    ...
    .attr('dx', (d) => d.value > 0 ?
      ...
      3)
    .attr('y', (d) => yScaleFunction(d.name) + yScaleFunction.bandwidth() / 2)
