...
function appendBaseMarker(svg, x, y, id) {
  return svg.append("circle")
    .attr("class", `annotation-marker base-marker ${id}`)
    .attr("cx", x)
    .attr("cy", y)
    .attr("r", 3)
    .style("fill", "green")
...
export function addAnnotationText(svgName,
  xValue,
  yValue, TextOffsetX, TextOffsetY, thisText, textAnchor, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false) {
...
export function addAnnotationArrow(svgName, xValue, yValue, arrowOffsetX, arrowOffsetY, xLength, yLength, curve, thisText, textPosition, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false) {
  ...
  if (mobileText != true || size == "lg") {
    //desktop
    svgName.append("path")
      .attr("class", `annotation_arrow ${annotationId}`)
      // .data()
      .attr("d", function (d) {
        return draw_curve(
    ...
    if (editable) {
      ...
      const tailHandle = svgName.append("circle")
        .attr("class", `annotation-handle tail-handle ${annotationId}`)
      ...
      const headHandle = svgName.append("circle")
        .attr("class", `annotation-handle head-handle ${annotationId}`)
...
export function addDirectionArrow(svgName,
  direction,
  arrowAnchor,
  xValue, yValue, thisText, wrapWidth, wrapVerticalAlign = "middle", arrowColour = "#414042", editable = false) {

  let textAnchor = 'start', textOffsetX = 0, textOffsetY = 5, thisPath;
  const annotationId = generateAnnotationId();
  ...
  svgName.append('path')
    .attr("d", thisPath)
    .attr("class", `direction-arrow ${annotationId}`)
 ...
 */
export function addAnnotationLineVertical(svgName, height, xValue, thisText, textPosition = 'right', yValue, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, moveToBack, size, editable = false) {
  ...
  svgName.append('line')
    ...
    .attr('y1', 0)
    .attr('y2', height)
    .attr('class', `annotation-line ${annotationId}`)
    .filter(function () {
 ...
 */
export function addAnnotationLineHorizontal(svgName, width, yValue, thisText, textPosition = below, xValue, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, moveToBack, size, editable = false) {
 ...
 * @param {string} size - Chart size ('sm', 'md', 'lg').
 */
export function addAnnotationRangeVertical(svgName, height, xValue, xEndValue, thisText, textPosition = "left", textPosition2 = "inside", yValue, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false) {
  ...
  svgName.append('rect')
    ...
    .attr('y', 0)
    .attr('height', height)
    .attr('class', `annotation-range ${annotationId}`)
 ...
 * @param {string} size - Chart size ('sm', 'md', 'lg').
 */
export function addAnnotationRangeHorizontal(svgName, width, yValue, yEndValue, thisText, textPosition = 'above', textPosition2 = 'inside', xValue, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false) {
  ...
  switch (type) {
    case 'arrow':
      return addAnnotationArrow(
        ...
        mob.number, mob.circleOffsetX, mob.circleOffsetY, sty.size, editable
      );
    ...
    case 'text':
      return addAnnotationText(
        ...
        mob.circleOffsetY, sty.size, editable
      );
    ...
    case 'line-vertical':
      return addAnnotationLineVertical(
        ...
        ln.moveToBack, sty.size, editable
      );
    ...
    case 'range-vertical':
      return addAnnotationRangeVertical(
        svg, ln.height, x, ln.endX, label, pos.text, pos.inside,
        y || 10, txt.wrapWidth, mob.enabled, mob.number,
        mob.circleOffsetX, mob.circleOffsetY, sty.size, editable
      );

    case 'range-horizontal':
      return addAnnotationRangeHorizontal(
        svg, ln.width, y, ln.endY, label, pos.alignment, pos.inside,
    ...
    case 'direction-arrow':
      return addDirectionArrow(
        ...
        txt.wrapWidth, txt.verticalAlign, sty.color, editable
      );
...
export function createAnnotationToolbar(selector, wholeSvg, chartContainer, scales, margin, width, height) {
  ...
  const annotationsNames = [
    {
      ...
      inputsNeeded: [
        { name: "x", default: 0 },
        { name: "y", default: 0 },
        { name: "label", default: "Default text" }
      ]
    },
    {
      label: "Arrow",
      ...
      inputsNeeded: [
        ...
        { name: "lengthX", default: 50 },
        { name: "lengthY", default: 50 },
        { name: "offsetX", default: 10 },
        { name: "offsetY", default: 10 }
      ]
    },
    {
    ...
    },
    {
      ...
      inputsNeeded: [
        ...
        { name: "endY", default: 100 },
        { name: "width", default: width },
        { name: "position", default: "above" },
        { name: "enclosure", default: "inside" }
      ]
    },
    {
      ...
      inputsNeeded: [
        ...
        { name: "endX", default: 100 },
        { name: "height", default: height },
        { name: "position", default: "left" },
        { name: "enclosure", default: "inside" }
      ]
    }
  ]
  ...
  fieldset.append('legend')
    .text("Choose a type of annotation")
  ...
  const dynamicInputsContainer = fieldset.append("div")
    .attr("id", "annotationInputs");

  inputs.selectAll('input[type="radio"]')
    .on("change", function () {
      const selectedValue = d3.select(this).property("value");
      renderAnnotationFields(selectedValue);
    });

  // Render defaults for the first selected type
  renderAnnotationFields(annotationsNames[0].name);

  d3.select(selector)
  .append("button")
  .attr('id','addwithbutton')
  .text('Click to add from fields')
  .on('click',function(){
    const selectedValue = d3.select('input[name="annotationTypes"]:checked').property("value");
    const typeDef = annotationsNames.find(d => d.name === selectedValue);

    const userInputs = {};
    typeDef.inputsNeeded.forEach(input => {
      const field = d3.select(`#${input.name}`);
      userInputs[input.name] = field.empty() ? input.default : field.property("value");
    });
    ...
    addAnnotation(annotationOptions);
    allAnnotations.push(annotationOptions);

    console.log("annotation saved:", annotationOptions);
  ...
  d3.select(selector)
    ...
    .on("click", function () {
      createClickDataRecorder(wholeSvg, scales, margin, (record) => {
        ...
        // Gather field values from the dynamic form
        const userInputs = {};
        typeDef.inputsNeeded
        .filter(d => d.name !== "x" && d.name !== "y")
        .forEach(input => {
          const field = d3.select(`#${input.name}`);
          userInputs[input.name] = field.empty() ? input.default : field.property("value");
        });
        ...
        addAnnotation(annotationOptions)
        allAnnotations.push({ ...record, ...annotationOptions });
  ...
  function renderAnnotationFields(annotationType) {
    const typeDef = annotationsNames.find(d => d.name === annotationType);

    if (!typeDef) return;

    const fields = dynamicInputsContainer.selectAll("div.input-field")
      .data(typeDef.inputsNeeded, d => d.name);

    // Remove old fields
    fields.exit().remove();

    // Add new fields
    const newFields = fields.enter()
      .append("div")
...
export function createAnnotationSystem(selector, wholeSvg, chartContainer, scales, margin, width, height) {
  const allAnnotations = [];
  
  const api = {
    getAnnotations: () => [...allAnnotations],
    clearAnnotations: () => allAnnotations.length = 0,
    removeAnnotation: (index) => allAnnotations.splice(index, 1)
  };
  
  // Create toolbar with access to api
  createAnnotationToolbar(selector, wholeSvg, chartContainer, scales, margin, width, height, api);
...
function createClickDataRecorder(svgContainer, scales, margin, callback) {
  ...
  return cancelClickMode()

}

/**
 * Inverts a scale for pixel → data mapping.
 * Supports both linear-like and band scales.
 */
