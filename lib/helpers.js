...
export function createAnnotationToolbar(selector, wholeSvg, chartContainer, scales, margin, width, height) {
  ...
  function renderAnnotationList(allAnnotations) {
    ...
    allAnnotations.forEach((annotation, index) => {
      ...
      buttons.append("button")
        ...
        .on("click", () => repositionWithClick(index))

      // Edit button
      buttons.append("button")
        .text("Edit position")
        .style("margin-right", "5px")
        .on("click", () => editAnnotationPosition(index, item));

      // Remove button
      buttons.append("button")
        .text("Remove annotation")
        .on("click", () => removeAnnotation(index));
    })

    annotationList.append("div")
      .append("button")
      .text("Save all annotations")
      .on("click", () => downloadJsonFile(allAnnotations,'annotations.json'))
  }

  function repositionWithClick(index) {
    createClickDataRecorder(wholeSvg, scales, margin, (record) => {
  ...
  function editAnnotationPosition(index, item) {
  ...
  function renderAnnotations(allAnnotations) {
    // remove all annotations
    wholeSvg.selectAll(".annotation").remove()
    allAnnotations.forEach(anno => addAnnotation(anno))
  }
 ...
 */
export async function loadAnnotationsFromJson(filePath) {
    try {
        ...
        jsonData.forEach(anno=>{
          addAnnotation(anno) // I suppose I need to tell it stuff like margins, scales, chartcontainer etc.
        })
