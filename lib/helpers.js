...
export function createDirectLabels({
    ...
    categories.forEach(function (category, index) {
        ...
        if (lastDatum[category] === null || lastDatum[category] === undefined) return;
        
        const label = svg.append('text')
            .attr('class', 'directLineLabel')
            .attr('x', xScale(lastDatum.date) + 10)
            .attr('y', yScale(lastDatum[category]))
            .attr('dy', '.35em')
            .attr('text-anchor', 'start')
        ...
        labelData.push({
            node: label,
            x: xScale(lastDatum.date) + 10,
            y: yScale(lastDatum[category]),
            originalY: yScale(lastDatum[category]),
            height: bbox.height,
            width: bbox.width,
            category: category,
            categoryIndex: index
