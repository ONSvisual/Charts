...
export function addAnnotationText(svgName,
  ...
  if (mobileText != true || size == "lg") {
    ...
    if (editable) {
      const baseMarker = svgName.append("circle")
        .attr("class", `annotation-marker base-marker ${annotationId}`)
        .attr("cx", xValue)
        .attr("cy", yValue)
        .attr("r", 3)
        .style("fill", "green")
...
function updateAnnotationPositions(config) {
  const { svgName, annotationId, xValue, yValue, TextOffsetX, TextOffsetY, textAnchor, centerWrap, wrapWidth, thisText } = config;

  // Update arrow path if it exists
  if (config.arrowConfig) {
    const { arrowOffsetX, arrowOffsetY, xLength, yLength, curveRight } = config.arrowConfig;
    svgName.select(`.annotation_arrow.annotation-${annotationId}`)
      .attr("d", draw_curve(
        xValue + arrowOffsetX,
        yValue + arrowOffsetY,
        xLength,
    ...
    svgName.select(`.tail-handle.annotation-${annotationId}`)
      .attr("cx", xValue + arrowOffsetX)
      .attr("cy", yValue + arrowOffsetY);

    svgName.select(`.head-handle.annotation-${annotationId}`)
  ...
  if (config.lineConfig) {
    const { height } = config.lineConfig;
    svgName.select(`.annotation-line.${annotationId}`)
      .attr('x1', xValue + 0.5)
      .attr('x2', xValue + 0.5)
      .attr('y2', height);
  }
  ...
  // Update base marker
  svgName.select(`.base-marker.${annotationId}`)
    .attr("cx", xValue)
    .attr("cy", yValue);

  // Update text position
  const textX = config.arrowConfig ?
    xValue + config.arrowConfig.xLength + TextOffsetX + config.arrowConfig.arrowOffsetX :
    xValue + TextOffsetX;
  const textY = config.arrowConfig ?
    yValue + config.arrowConfig.yLength + TextOffsetY + config.arrowConfig.arrowOffsetY :
    yValue + (TextOffsetY || 0);
...
function openAnnotationModal(config) {
  ...
  // Conditionally add text position select
  let positionSelect;
  if (config.showPosition && config.positionOptions) {
    modalContent.append("label").text("Text Position:");
    positionSelect = modalContent.append("select")
      .style("width", "100%")
      .style("margin", "10px 0")
      .style("padding", "5px");

    config.positionOptions.forEach(pos => {
      positionSelect.append("option")
        .attr("value", pos)
        .property("selected", pos === config.currentPosition)
        .text(pos);
    });
  }

  // Conditionally add curve direction
  let curveSelect;
  if (config.showCurve) {
    modalContent.append("label").text("Curve Direction:");
    curveSelect = modalContent.append("select")
      .style("width", "100%")
      .style("margin", "10px 0")
      .style("padding", "5px");

    curveSelect.append("option").attr("value", "left").property("selected", config.currentCurve).text("Left");
    curveSelect.append("option").attr("value", "right").property("selected", !config.currentCurve).text("Right");
  }
  ...
  buttonContainer.append("button")
    ...
    .on("click", function () {
      ...
      if (config.showPosition && positionSelect) {
        newValues.position = positionSelect.property("value");
      }

      if (config.showCurve) {
        newValues.curve = curveSelect.property("value") === "left";
      }
...
export function addAnnotationArrow(svgName, xValue, yValue, arrowOffsetX, arrowOffsetY, xLength, yLength, curveRight, thisText, textPosition, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false) {
      ...
      svgName.select(`.annotation-text.${annotationId}`)
        ...
        .on("click", function (event) {
          ...
          openAnnotationModal({
            ...
            showPosition: true,
            showCurve: true,
            positionOptions: ["above", "below", "left", "right"],
            onApply: function (newValues) {
              thisText = newValues.text;
              textPosition = newValues.position;
              curveRight = newValues.curve;
              wrapWidth = newValues.wrapWidth;
              ...
              updateAnnotationPositions({
                svgName, annotationId, xValue, yValue, TextOffsetX, TextOffsetY,
                textAnchor, centerWrap, wrapWidth, thisText,
                arrowConfig: { arrowOffsetX, arrowOffsetY, xLength, yLength, curveRight }
              });
              svgName.select(`.annotation-text.${annotationId}`)
                .text(thisText)
                .attr('text-anchor', textAnchor)
        ...
        .on("click", function (event) {
          ...
          openAnnotationModal({
            ...
            onApply: function (newValues) {
              ...
              updateAnnotationPositions({
                svgName, annotationId, xValue, yValue, TextOffsetX, TextOffsetY: 0, 
                textAnchor, centerWrap, wrapWidth, thisText,
                lineConfig: { height }
              });
 ...
 */
export function addSimpleAnnotation(svg, type, x, y, text, options = {}) {
 ...
 * @param {string} [config.arrow.bendDirection] - Elbow bend: 'horizontal-first', 'vertical-first'
 * @param {number} [config.arrow.endX] - Elbow arrow end X
 * @param {number} [config.arrow.endY] - Elbow arrow end Y
 ...
 * @param {number} [config.text.adjustY] - Y adjustment for text
 * @param {string} [config.text.verticalAlign] - Vertical alignment: 'top', 'middle', 'bottom'
...
export function addAnnotation(config) {
  ...
  const txt = { ...defaults.text, ...textConfig };
  ...
  switch (type) {
    case 'arrow':
      return addAnnotationArrow(
        ...
        arr.curve, text, pos.text, txt.wrapWidth, mob.enabled,
        mob.number, mob.circleOffsetX, mob.circleOffsetY, sty.size, editable
      );

    case 'text':
      return addAnnotationText(
        svg, x, y, arr.offsetX || 0, arr.offsetY || 0, text,
    ...
    case 'line-vertical':
      return addAnnotationLineVertical(
        svg, ln.height, x, text, pos.text, y || 10, txt.wrapWidth,
    ...
    case 'line-horizontal':
      return addAnnotationLineHorizontal(
        svg, ln.width, y, text, pos.text, x || 10, txt.wrapWidth,
        mob.enabled, mob.number, mob.circleOffsetX, mob.circleOffsetY,
        ln.moveToBack, sty.size
      );

    case 'range-vertical':
      return addAnnotationRangeVertical(
        svg, ln.height, x, ln.endX, text, pos.text, pos.inside,
        y || 10, txt.wrapWidth, mob.enabled, mob.number,
        mob.circleOffsetX, mob.circleOffsetY, sty.size
      );

    case 'range-horizontal':
      return addAnnotationRangeHorizontal(
        svg, ln.width, y, ln.endY, text, pos.text, pos.inside,
    ...
    case 'direction-arrow':
      return addDirectionArrow(
        svg, arr.direction, pos.anchor, x, y, pos.alignment, text,
        txt.wrapWidth, txt.adjustY, txt.verticalAlign, sty.color
      );

    case 'elbow-arrow':
      return addElbowArrow(
        svg, x, y, arr.endX, arr.endY, arr.bendDirection, pos.anchor,
        text, txt.wrapWidth, txt.adjustY, txt.verticalAlign, sty.color,
        pos.text
      );
