...
export function addAnnotationArrow(svgName, xValue, yValue, arrowOffsetX, arrowOffsetY, xLength, yLength, curveRight, thisText, textPosition, wrapWidth, mobileText, mobileTextNumber, MobileCircleOffsetX, MobileCircleOffsetY, size, editable = false) {
  ...
  if (
    ...
    svgName.append("path")
      .attr("class", `annotation_arrow annotation-${annotationId}`)
      // .data()
      .attr("d", function (d) {
        return draw_curve(
    ...
    svgName.append('g')
      ...
      // .attr('dy', yScale(yValue) + yLength + TextOffsetY + arrowOffsetY)
      .attr('class', `annotation-text annotation-${annotationId}`)
      .text(thisText)
      .attr('text-anchor', textAnchor)
      .call(wrap2, wrapWidth, 0.35, 1.1, 1, true, centerWrap);
    ...
    if (editable) {
      ...
      svgName.select(`.annotation-text.annotation-${annotationId}`)
        .style("cursor", "pointer")
        .on("click", function (event) {
          console.log(event)
          event.stopPropagation();
          if (editable) openAnnotationModal();
        });

      // Add base position marker
      const baseMarker = svgName.append("circle")
        .attr("class", `annotation-marker base-marker annotation-${annotationId}`)
        .attr("cx", xValue)
        .attr("cy", yValue)
        .attr("r", 3)
        .style("fill", "green")
      ...
      const tailHandle = svgName.append("circle")
        .attr("class", `annotation-handle tail-handle annotation-${annotationId}`)
        ...
        .call(d3.drag()
          .on("drag", function (event) {
            ...
            arrowOffsetX = newX;
            arrowOffsetY = newY;
            if (editable) updatePositions();
          })
        );

      // Add head handle
      const headHandle = svgName.append("circle")
        .attr("class", `annotation-handle head-handle annotation-${annotationId}`)
    ...
    function updatePositions() {
      ...
      svgName.select(`.annotation-text.annotation-${annotationId}`).selectAll('tspan')
        .attr('x', xValue + xLength + TextOffsetX + arrowOffsetX)
        .attr('y', yValue + yLength + TextOffsetY + arrowOffsetY);
    }
    ...
    function openAnnotationModal() {
      ...
      modalContent.append("h3").text("Edit Annotation");
      ...
      ["above", "below", "left", "right"].forEach(pos => {
        positionSelect.append("option")
          .attr("value", pos)
          .property("selected", pos === textPosition)
          .text(pos);
      });

      // Curve direction
      modalContent.append("label").text("Curve Direction:");
      const curveSelect = modalContent.append("select")
        .style("width", "100%")
        .style("margin", "10px 0")
        .style("padding", "5px");

      curveSelect.append("option").attr("value", "left").property("selected", curveRight).text("Left");
      curveSelect.append("option").attr("value", "right").property("selected", !curveRight).text("Right");

      // Wrap width input
      modalContent.append("label").text("Text Wrap Width:");
      const wrapInput = modalContent.append("input")
        .attr("type", "number")
        .attr("value", wrapWidth)
        .style("width", "100%")
        .style("margin", "10px 0")
        .style("padding", "5px");
      ...
      buttonContainer.append("button")
        ...
        .on("click", function () {
          // Update values
          thisText = textInput.property("value");
          textPosition = positionSelect.property("value");
          curveRight = curveSelect.property("value") === "left";
