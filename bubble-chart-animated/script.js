...
function drawGraphic() {
  ...
  function makeSlider() {
   ...
   sliderDomain = [0, timepoints.length-1]; //set the domain according to the length of the timepoints
    ...
    .displayFormat(function(i) { return dateformat(dateparse(timepoints[i]))}) //labels taken from timepoints
    .displayValue(true)
    .handle(
      d3.symbol()
        .type(d3.symbolCircle)
        .size(500)
    )//Handle colour is set in the chart.css file under parameter handle
    .fill("#206095")
    .ticks(0)
    .on('onchange', function(val) {
      ...
      updateVisuals(graphic_data.filter(function(d) {
...
// console.log(d3.pointer(event))
  if(parseInt(graphic.style("width")) > threshold_md){
    tooltip
    .html('<span style ="color: #206095;font-size: 15px;">'+ d.group +'</span>'+"<br><br>"+'<span style="font-weight:500; opacity:1">'+"Wage growth: "+ data_format(d.y)+" p.p."+'</span>'+"<br>"+'<span style="font-weight:500; opacity:1">'+"Median hourly pay (£): "+ data_format(d.x)+"%"+'</span>')
    .style("left", d3.pointer(event)[0] > chart_width-200 ? d3.pointer(event)[0]-200 + "px" : d3.pointer(event)[0]+25 + "px")		
    .style("top", d3.pointer(event)[1] < 25 ? d3.pointer(event)[1]+150 + "px" : d3.pointer(event)[1]-20 + "px")		
...
svg.selectAll('circle')
 ...
 function drawLegend() {
  ...
  //pull in the legend data from the config
   ...
   let legendData = d3.zip(config.essential.legendLabels, config.essential.legendRadius,config.essential.legendCX)
...
if((parseInt(graphic.style("width")) > threshold_md && config.essential.highlight === true) === true ){
  drawHighlight()
...
function drawHighlight() {
  ...
  .attr("id",function(d,i) {return d.group})
  .style('font-size',"14px")
  .style('font-weight',500)
  .attr('x',function(d) { if (d.label_y == "middle" && d.label_anchor =="start") {
                            return x(d.x)+r(d.size)+6}// shifts to the side of the circle when text anchor is middle
                            else if (d.label_y == "middle" && d.label_anchor =="end"){
                              return x(d.x)-r(d.size)-6}// shifts to the other side of the circle when text anchor is middle
                          else {return x(d.x)}
                        })
  // .attr('y',function(d) { return (y(d.y) < (topYTick) ? (y(d.y)+r(d.size)+15) : (y(d.y)-r(d.size))-2 )})
  .attr('y',function(d) { if (d.label_y == "top") {return y(d.y)-r(d.size)-6} 
                          else if (d.label_y == "bottom") {return y(d.y)+r(d.size)+15}
                          else if (d.label_y == "middle") {return y(d.y)}
                          else {return y(d.y)-r(d.size)-2}
                        })
  .style('text-anchor',function(d) { if (d.label_anchor == "start") {return d.label_anchor} 
                                  else if (d.label_anchor == "middle") {return d.label_anchor} 
                                  else if (d.label_anchor == "end") {return d.label_anchor} 
                                  else {return "start"}
                                  })
