...
function drawGraphic() {
  ...
  var height = (config.optional.seriesHeight[size] * (graphic_data.length / 2)) + (10 * ((graphic_data.length / 2) - 1)) + 12
    ...
    for (let j = 0; j < groups.length; j++) {
      if (i == j) {
        d3.select(this)
          ...
          .join("rect")
          .attr("x", (d) => x(d[0]))
          .attr("y", (d) => groups[i][3](d.data.name) + groups[i][6](d.data.sex))
  ...
  // This does the x-axis label - here only added to the last group
    ...
    d3.select(this)
      ...
      .append('text')
      .attr('x', 5)
      .attr('y', groups[i][3].paddingOuter() * groups[i][3].bandwidth() + groups[i][6].paddingOuter() * groups[i][6].bandwidth())
    ...
    d3.select(this)
      ...
      .append('text')
      .attr('x', 5)
      .attr('y', groups[i][3].paddingOuter() * groups[i][3].bandwidth() + groups[i][6].bandwidth() + groups[i][6].paddingInner() * groups[i][6].bandwidth())
  ...
  var legenditem = d3.select('#legend')
    ...
    .data(d3.zip(graphic_data.columns.slice(3), config.essential.colour_palette))
    .enter()
    .append('div')
  ...
  legenditem.append('div')
    .append('p').attr('class', 'legend--text').html(d => d[0])
